...
func (ns *ns) onNewDstPodSelector(selector *selector) error {
	...
	for _, pod := range ns.pods {
		if hasIP(pod) {
			...
			if selector.matches(pod.ObjectMeta.Labels) {
				if err := ns.ips.DelEntryIfExists(ns.defaultAllowIPSet, pod.Status.PodIP, pod.Name); err != nil {
					return err
				}
			}
		}
...
func (ns *ns) addToDefaultAllowIfNoMatching(pod *coreapi.Pod) error {
	...
	if !found {
		if err := ns.ips.AddEntryIfNotExist(ns.defaultAllowIPSet, pod.Status.PodIP, pod.Name, podComment(pod)); err != nil {
			return err
		}
	}
	return nil
...
func (ns *ns) addPod(obj *coreapi.Pod) error {
	...
	if !ns.legacy && !found {
		if err := ns.ips.AddEntryIfNotExist(ns.defaultAllowIPSet, obj.Status.PodIP, obj.Name, podComment(obj)); err != nil {
			return err
		}
	}
...
func (ns *ns) updatePod(oldObj, newObj *coreapi.Pod) error {
	...
	if hasIP(oldObj) && !hasIP(newObj) {
		...
		if !ns.legacy {
			if err := ns.ips.DelEntryIfExists(ns.defaultAllowIPSet, oldObj.Status.PodIP, oldObj.Name); err != nil {
				return err
			}
		}
	...
	if !hasIP(oldObj) && hasIP(newObj) {
		...
		if !ns.legacy && !found {
			if err := ns.ips.AddEntryIfNotExist(ns.defaultAllowIPSet, newObj.Status.PodIP, newObj.Name, podComment(newObj)); err != nil {
				return err
			}
		}
	...
	if !ns.legacy &&
		...
		ns.ips.Exist(ns.defaultAllowIPSet, oldObj.Status.PodIP, oldObj.Name) {
...
func (ns *ns) deletePod(obj *coreapi.Pod) error {
	...
	if !ns.legacy {
		if err := ns.ips.DelEntryIfExists(ns.defaultAllowIPSet, obj.Status.PodIP, obj.Name); err != nil {
			return err
		}
	}
